...
public:
	...
	long MouseOverRowColor = RGB(234,246,254)		//RGB(230,226,220)
...
for i = 1 to ll_objcnt
			...
			if ls_band = 'detail' then
				...
				if SetDisabledColumnBackgroundColor = true and 	(left(ls_bgcolor, 9) = '553648127' or left(ls_bgcolor, 9) = '536870912') then
					if pos(ls_bgcolor, '~t') = 0 then
						...
						// Tabular는 TabOrder=0일 때 DisabledColor 적용 안 함
						if ls_protect <> '!' and ls_protect <> '?' and ls_protect <> '' and ls_protect <> '0' then
							ll_pos = pos(ls_protect, '~t')
							
							if ll_pos > 0 then
								ls_protect = mid(ls_protect, ll_pos + 1, len(ls_protect) - ll_pos - 1)
								ls_colbgcolor = ls_bgcolor + of_getinnersyntax('~t If(Long(Describe("Evaluate(~~~"" + "' + ls_protect + '" + "~~~", " + String(GetRow()) + ")")) = 0 And Long(Describe("' + ls_objects[i] + '.TabSequence")) > 0, ' + string(EditableColumnBackgroundColor) + ', ' + string(DisabledColumnBackgroundColor) + ')' )
	...
	if ls_border = '2' then
		...
		if ls_colbgcolor <> '' then
			lnv_newsyntax.of_append('rectangle(band=' + ls_band + ' x="' +String(ll_cellxpos) + '" y="' + string(ll_cellypos) + '" height="' + string(ll_cellheight) + '"' + ' width="' + string(ll_cellwidth) + '" name='  + ls_objects[i] + '_cellborder  visible="' + ls_visible + '" brush.hatch="6" brush.color="' + ls_colbgcolor + '"' +  &
			...
			lnv_newsyntax.of_append('rectangle(band=' + ls_band + ' x="' +String(ll_cellxpos) + '" y="' + string(ll_cellypos) + '" height="' + string(ll_cellheight) + '"' + ' width="' + string(ll_cellwidth) + '" name='  + ls_objects[i] + '_cellborder  visible="' + ls_visible + '" brush.hatch="7" brush.color="' + ls_bgcolor + '"' +  &
												' pen.style="0" pen.width="' + string(PixelsToUnits(CellBorderThickNess, YPixelsToUnits!)) + '" pen.color="' + string(CellBorderColor) + '" background.mode="1" background.color="553648127" )')
		end if
			...
			case 'column', 'compute'
				...
				if ls_editstyle = 'dddw' or ls_editstyle = 'ddlb' then
					if ll_cellheight > ll_height + pixelstounits(2, ypixelstounits!) then
						...
						lnv_modsyntax.of_append('create line(band=detail x1="' + string(ll_xpos) + '" y1="' + string(ll_ypos) + '" x2="' + string(ll_xpos + ll_width + pixelstounits(1, xpixelstounits!)) + '" y2="' + string(ll_ypos) + '"  name=' + ls_objects[i] + '_border1' + ' visible="' + ls_visible + '" pen.style="0" pen.width="5" pen.color="' + ls_bgcolor + '"  background.mode="2" background.color="33554432")')
						lnv_modsyntax.of_append('create line(band=detail x1="' + string(ll_xpos) + '" y1="' + string(ll_ypos + ll_height) + '" x2="' + string(ll_xpos + ll_width + pixelstounits(1, xpixelstounits!)) + '" y2="' + string(ll_ypos + ll_height) + '"  name=' + ls_objects[i] + '_border2' + ' visible="' + ls_visible + '" pen.style="0" pen.width="5" pen.color="' + ls_bgcolor + '"  background.mode="2" background.color="33554432")')
						lnv_modsyntax.of_append('create line(band=detail x1="' + string(ll_xpos) + '" y1="' + string(ll_ypos) + '" x2="' + string(ll_xpos) + '" y2="' + string(ll_ypos + ll_height + pixelstounits(1, ypixelstounits!)) + '"  name=' + ls_objects[i] + '_border3' + ' visible="' + ls_visible + '" pen.style="0" pen.width="5" pen.color="' + ls_bgcolor + '"  background.mode="2" background.color="33554432")')
						lnv_modsyntax.of_append('create line(band=detail x1="' + string(ll_xpos + ll_width) + '" y1="' + string(ll_ypos) + '" x2="' + string(ll_xpos + ll_width) + '" y2="' + string(ll_ypos + ll_height + pixelstounits(1, ypixelstounits!)) + '"  name=' + ls_objects[i] + '_border4' + ' visible="' + ls_visible + '" pen.style="0" pen.width="5" pen.color="' + ls_bgcolor + '"  background.mode="2" background.color="33554432")')
					end if
				end if
		end choose
	end if	
